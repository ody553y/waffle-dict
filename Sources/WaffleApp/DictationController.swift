...
final class DictationController: ObservableObject {
    ...
    func dismissAccessibilityPrompt() {
        shouldShowAccessibilityPrompt = false
        UserDefaults.standard.set(true, forKey: "waffle.didDismissAccessibilityPrompt")
    }
    ...
    private func updateAccessibilityPromptState(for pasteResult: PasteHelper.Result) {
        ...
        let didDismissPrompt = UserDefaults.standard.bool(forKey: "waffle.didDismissAccessibilityPrompt")
    ...
    private func shouldBackupToICloud() -> Bool {
        value(for: "iCloudBackupEnabled", defaultValue: false)
    }
        ...
        if let urlError = error as? URLError {
            ...
            case .cannotConnectToHost, .cannotFindHost, .networkConnectionLost:
                return localized(
                    ...
                    default: "Worker is not running. Restart Waffle.",
    ...
    private func transcribeRecording(
        ...
        do {
            ...
            triggerAutoExportPipelinesIfNeeded(for: savedRecord)
            ...
            if didRestart {
                ...
                state = .error(
                    localized(
                        ...
                        default: "Worker crashed. Restart failed. Restart Waffle.",
    ...
    private func saveTranscriptToHistory(
        ...
        return await Task.detached(priority: .utility) {
            do {
                ...
                print("[Waffle] Failed to save transcript history: \(error)")
                return nil
            }
        }.value
    }
    ...
    private func finalizeRecordingStorage(recordingURL: URL, savedRecord: TranscriptRecord?) async {
        ...
        do {
            ...
            print("[Waffle] Failed to archive retained audio: \(error)")
            audioCaptureService.cleanupScratchFile(recordingURL)
        }
    }
    ...
    private func triggerAutoExportPipelinesIfNeeded(for transcript: TranscriptRecord?) {
        guard let transcript else { return }
        ...
        guard let actionService = makeTranscriptActionService() else {
            Self.log.debug("Auto-pipelines skipped because LM Studio is not configured")
            return
        }

        let autoPipelines = exportPipelineStore.load().filter(\.runAutomatically)
        guard autoPipelines.isEmpty == false else { return }
        ...
        for pipeline in autoPipelines {
            Task.detached(priority: .utility) {
                do {
                    ...
                    await Self.applyOutputDestination(finalText, destination: pipeline.outputDestination)
                } catch {
                    logger.debug(
                        "Auto-pipeline failed for \(pipeline.id.uuidString, privacy: .public): \(String(describing: error), privacy: .public)"
                    )
                }
            }
        }
    ...
    private func triggerICloudBackupIfNeeded(for transcript: TranscriptRecord?) {
        guard shouldBackupToICloud() else { return }
        guard let transcriptStore else { return }
        guard let transcript else { return }
    ...
    @MainActor
    private static func applyOutputDestination(
        _ text: String,
        destination: ExportPipelineOutputDestination
    ) {
        switch destination {
        case .clipboard:
