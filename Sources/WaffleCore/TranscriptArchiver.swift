...
public struct WaffleArchive: Codable, Equatable, Sendable {
...
public struct TranscriptArchiver: Sendable {
    ...
    public func `import`(from data: Data) throws -> WaffleArchive {
        let archive: WaffleArchive
        do {
            archive = try Self.makeDecoder().decode(WaffleArchive.self, from: data)
        } catch {
    ...
    public func write(_ archive: WaffleArchive, to url: URL) throws {
        let data: Data
        do {
    ...
    public func read(from url: URL) throws -> WaffleArchive {
        let data: Data
        do {
