...
struct LocalizationCatalogTests {
    @Test func localizableCatalogExists() throws {
        let path = repositoryRootPath()
            .appendingPathComponent("Sources/ScreamerApp/Localizable.xcstrings")
            .path
        #expect(FileManager.default.fileExists(atPath: path))
    }

    @Test func everyStringHasEnglishGermanAndJapaneseTranslations() throws {
        let url = repositoryRootPath().appendingPathComponent("Sources/ScreamerApp/Localizable.xcstrings")
